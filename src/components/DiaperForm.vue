...
const diaperTypes = [
  { value: 'wet', label: '💧 Xixi', color: 'blue' },
  { value: 'soiled', label: '💩 Cocô', color: 'brown' },
  { value: 'both', label: '💧💩 Ambos', color: 'purple' },
]
  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-background-surface border border-border rounded-2xl max-w-md w-full">
      <div class="flex items-center justify-between p-6 border-b border-border">
        <h2 class="text-xl font-bold text-white">Registrar Troca de Fralda</h2>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        <div>
          ...
          <div class="grid grid-cols-3 gap-2">
            ...
            >
              <div class="text-2xl">{{ type.label }}</div>
            </button>
          </div>
        </div>
